...
/////////////////////////////////////////validacion nombre 
 ...
 jQuery(function($){
   ...
   $('#salario').on('keypress', function (e) {/////////validacion de numeros con dos decimales
	...
	function r(id) { location.href="bajaPastor.php" } 
	function mandar(id,nombre,apellido) { 
		alertify.confirm("<center>ATENCI&Oacute;N!</center>", "<center><img src='../img/warning.png' width='230' height='230'></center>"+"<center><h1>Desea dar de baja al pastor </h1><h2 style='font-style:italic';>"+nombre+" "+apellido+" ?</h2></center>    ", 
			...
			<!--aqui va la foto del usuario -->
				...
				<br>
				 <li style="color:#fff; cursor:default;">
                    <h4><span class="all-tittles">Baja Pastor</span></h4>
                </li>
				</center>
...
<table id="grid" class="display table-bordered" cellspacing="0" width="101%">
    <thead>
      <tr>
...
<tbody>
    ...
    <?php $conect=pg_query($conexion,"select distinct p.idpastor,p.nombre,p.apellido,p.fechanacimiento,p.estadocivil,p.dui,p.duiesposa,p.telefono,p.direccion,p.correo,p.fechain from pastor as p,cargo as c,cargo_pastor as cp where c.nivel!=0 and c.nivel!=1 and cp.idpastor=p.idpastor and c.idcargo=cp.idcargo"); 
      ...
      <tr>
							...
							?></td>
                            <td align="left"><font size="1" ></font><?php echo $vector[5]; ?></td>
                       ...
                       ?>
                            ...
                            if (pg_num_rows($conect2)) { ?>
                            	<td align="left"><font size="4" ></font><button type="button" id="cancelar" name="cancelar" onClick="mandar('<?php echo $vector[0]; ?>','<?php echo $vector[1]; ?>','<?php echo $vector[2]; ?>')" class="btn btn-success">Dar baja</button></td>
                           <?php }else{  ?>
                            	<td align="left"><font size="4" ></font>No Asignado</td>
...
pg_query("BEGIN");	
       ...
       $result=pg_query($conexion,"UPDATE iglesia SET  idpastor=null WHERE idpastor=$idpastor" );

       $result=pg_query($conexion,"DELETE FROM cargo_pastor WHERE idpastor=$idpastor" ); 
       $result=pg_query($conexion,"DELETE FROM detalle_senora WHERE idpastor=$idpastor" );  

	   $result=pg_query($conexion,"insert into cargo_pastor(idcargo,idpastor) values(4,$idpastor)" );  
